...
function App() {
 ...
 <header className="bg-white shadow-sm px-3 sm:px-4 pb-3 sm:pb-4 landscape:pb-2 pt-[calc(env(safe-area-inset-top)+0.75rem)] sm:pt-[calc(env(safe-area-inset-top)+1rem)] landscape:pt-[calc(env(safe-area-inset-top)+0.5rem)] flex items-center justify-between sticky top-0 z-10">
 ...
 <Brain className="text-indigo-600 shrink-0 w-6 h-6 sm:w-7 sm:h-7 " />
 <h1 className="text-lg sm:text-xl font-bold text-slate-800 truncate shrink-1">치매 예방 두뇌 게임</h1>
 <button 
 ...
 <div className="hidden sm:flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm font-medium text-slate-600 bg-slate-100 px-2 sm:px-3 py-1 sm:py-1.5 rounded-full">
 ...
 <main className="flex-1 flex flex-col items-center justify-center p-4 sm:p-8 landscape:p-2 landscape:py-0">
 ...
 {currentScreen === 'NUMBER' && <LandscapeGameWrapper key="number_wrapper" title="숫자 기억 게임" description="화면에 나타난 숫자들을 1부터 순서대로 빠르게 터치하세요. 기억력과 순발력을 기를 수 있습니다." onBack={() => navigateTo('HOME')}><NumberGame key="number" initialLevel={getSavedLevel('NUMBER')} onSaveLevel={(l) => saveLevel('NUMBER', l)} onFinish={(s) => saveScore('숫자 기억', s)} /></LandscapeGameWrapper>}
 {currentScreen === 'PICTURE' && <LandscapeGameWrapper key="picture_wrapper" title="그림 맞추기" description="카드를 뒤집어 같은 그림을 짝지어보세요. 단기 기억력 향상에 도움이 됩니다." onBack={() => navigateTo('HOME')}><PictureGame key="picture" initialLevel={getSavedLevel('PICTURE')} onSaveLevel={(l) => saveLevel('PICTURE', l)} onFinish={(s) => saveScore('그림 맞추기', s)} /></LandscapeGameWrapper>}
 {currentScreen === 'PUZZLE' && <LandscapeGameWrapper key="puzzle_wrapper" title="슬라이딩 퍼즐" description="빈칸을 이용해 조각들을 이동시켜 원래 그림이나 숫자의 순서대로 맞춰보세요. 공간 지각력과 문제 해결 능력을 키웁니다." onBack={() => navigateTo('HOME')}><PuzzleGame key="puzzle" initialLevel={getSavedLevel('PUZZLE')} onSaveLevel={(l) => saveLevel('PUZZLE', l)} onFinish={(s) => saveScore('슬라이딩 퍼즐', s)} /></LandscapeGameWrapper>}
 {currentScreen === 'COLOR_WORD' && <LandscapeGameWrapper key="color_word_wrapper" title="색상 단어 게임 (스트룹)" description="글자의 의미가 아닌, 글자가 칠해진 '색상'을 맞추는 게임입니다. 인지 유연성과 집중력을 높여줍니다." onBack={() => navigateTo('HOME')}><ColorWordGame key="color_word" initialLevel={getSavedLevel('COLOR_WORD')} onSaveLevel={(l) => saveLevel('COLOR_WORD', l)} onFinish={(s) => saveScore('색상 단어', s)} /></LandscapeGameWrapper>}
 {currentScreen === 'PATTERN' && <LandscapeGameWrapper key="pattern_wrapper" title="패턴 기억 게임" description="화면에 나타나는 패턴의 순서를 기억하고 그대로 따라 누르세요. 작업 기억력을 향상시킵니다." onBack={() => navigateTo('HOME')}><PatternGame key="pattern" initialLevel={getSavedLevel('PATTERN')} onSaveLevel={(l) => saveLevel('PATTERN', l)} onFinish={(s) => saveScore('패턴 기억', s)} /></LandscapeGameWrapper>}
 {currentScreen === 'MATH' && <LandscapeGameWrapper key="math_wrapper" title="두뇌 회전 사칙연산" description="제시된 사칙연산 문제를 빠르게 암산하여 정답을 맞추세요. 계산 능력과 두뇌 회전 속도를 높입니다." onBack={() => navigateTo('HOME')}><MathGame key="math" initialLevel={getSavedLevel('MATH')} onSaveLevel={(l) => saveLevel('MATH', l)} onFinish={(s) => saveScore('사칙연산', s)} /></LandscapeGameWrapper>}
 {currentScreen === 'ODD_ONE_OUT' && <LandscapeGameWrapper key="odd_one_out_wrapper" title="다른 하나 찾기" description="여러 개의 그림 중 미세하게 다른 하나를 찾아내세요. 시각적 주의력과 관찰력을 기릅니다." onBack={() => navigateTo('HOME')}><OddOneOutGame key="odd_one_out" initialLevel={getSavedLevel('ODD_ONE_OUT')} onSaveLevel={(l) => saveLevel('ODD_ONE_OUT', l)} onFinish={(s) => saveScore('다른 하나 찾기', s)} /></LandscapeGameWrapper>}
 {currentScreen === 'RPS' && <LandscapeGameWrapper key="rps_wrapper" title="가위바위보 두뇌" description="화면에 제시된 조건(이기기, 지기, 비기기)에 맞춰 올바른 가위바위보 손모양을 빠르게 선택하세요. 판단력과 순발력을 키웁니다." onBack={() => navigateTo('HOME')}><RPSBrainGame key="rps" initialLevel={getSavedLevel('RPS')} onSaveLevel={(l) => saveLevel('RPS', l)} onFinish={(s) => saveScore('가위바위보 두뇌', s)} /></LandscapeGameWrapper>}
 {currentScreen === 'INITIAL_QUIZ' && <LandscapeGameWrapper key="initial_quiz_wrapper" title="초성 퀴즈" description="주어진 초성을 보고 알맞은 단어를 유추하여 맞추세요. 어휘력과 연상 능력을 향상시킵니다." onBack={() => navigateTo('HOME')}><InitialQuizGame key="initial_quiz" initialLevel={getSavedLevel('INITIAL_QUIZ')} onSaveLevel={(l) => saveLevel('INITIAL_QUIZ', l)} onFinish={(s) => saveScore('초성 퀴즈', s)} /></LandscapeGameWrapper>}
 {currentScreen === 'FLANKER' && <LandscapeGameWrapper key="flanker_wrapper" title="방해꾼 화살표" description="가운데 화살표가 가리키는 방향을 맞추세요. 주변 화살표의 방해를 이겨내며 집중력과 억제 능력을 기릅니다." onBack={() => navigateTo('HOME')}><FlankerTaskGame key="flanker" initialLevel={getSavedLevel('FLANKER')} onSaveLevel={(l) => saveLevel('FLANKER', l)} onFinish={(s) => saveScore('방해꾼 화살표', s)} /></LandscapeGameWrapper>}
 </AnimatePresence>
 </main>
...
function HomeScreen({ onNavigate, currentUser }: { onNavigate: (s: Screen) => void, currentUser: UserData }) {
 ...
 animate={{ opacity: 1, y: 0 }}
 exit={{ opacity: 0, y: -20 }}
 className="w-full max-w-md landscape:max-w-5xl flex flex-col gap-3 pb-8 "
 >
...
function LoginScreen({ onLogin }: { onLogin: (user: UserData) => void }) {
 ...
 return (
 <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="w-full max-w-md landscape:max-w-3xl bg-white rounded-3xl shadow-xl overflow-hidden border border-slate-100">
 <div className="flex border-b border-slate-100">
 <button 
 onClick={() => setTab('CENTER')}
 className={clsx("flex-1 py-4 landscape:py-2 text-center font-bold transition-colors", tab === 'CENTER' ? "bg-indigo-50 text-indigo-600 border-b-2 border-indigo-600" : "text-slate-500 hover:bg-slate-50")}
 >
 센터 로그인
 </button>
 <button 
 onClick={() => setTab('INDIVIDUAL')}
 className={clsx("flex-1 py-4 landscape:py-2 text-center font-bold transition-colors", tab === 'INDIVIDUAL' ? "bg-indigo-50 text-indigo-600 border-b-2 border-indigo-600" : "text-slate-500 hover:bg-slate-50")}
 >
 개인전 참가
 </button>
 </div>

 <div className="p-6 sm:p-8 landscape:p-3 landscape:pb-2">
 {tab === 'INDIVIDUAL' ? (
 <form onSubmit={handleIndividualSubmit} className="flex flex-col gap-4 landscape:gap-2">
 <div className="landscape:grid landscape:grid-cols-3 landscape:gap-3">
 <div>
 <label className="block text-sm landscape:text-xs font-bold text-slate-700 mb-1 landscape:mb-0.5">이름</label>
 <input type="text" required value={name} onChange={e => setName(e.target.value)} placeholder="홍길동" className="w-full p-3 landscape:p-1.5 landscape:text-sm rounded-xl border border-slate-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all" />
 </div>
 <div>
 <label className="block text-sm landscape:text-xs font-bold text-slate-700 mb-1 landscape:mb-0.5">생년월일 (6자리)</label>
 <input type="text" required value={birth} onChange={e => setBirth(e.target.value)} placeholder="450101" maxLength={6} className="w-full p-3 landscape:p-1.5 landscape:text-sm rounded-xl border border-slate-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all" />
 </div>
 <div>
 <label className="block text-sm landscape:text-xs font-bold text-slate-700 mb-1 landscape:mb-0.5">소속 센터명</label>
 <input type="text" required value={center} onChange={e => setCenter(e.target.value)} placeholder="행복치매예방센터" className="w-full p-3 landscape:p-1.5 landscape:text-sm rounded-xl border border-slate-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all" />
 </div>
 </div>
 {recentIndividuals.length > 0 && (
 <div className="landscape:mt-1">
 <label className="block text-xs font-bold text-slate-500 mb-2 landscape:mb-1">최근 로그인한 사용자</label>
 ...
 className="px-3 py-1.5 landscape:py-1 bg-slate-100 text-slate-600 text-sm rounded-full hover:bg-indigo-50 hover:text-indigo-600 transition-colors border border-slate-200"
 >
 {ind.name} ({ind.center})
 </button>
 ))}
 ...
 <button type="submit" className="mt-4 landscape:mt-1 w-full py-4 landscape:py-2 bg-indigo-600 text-white rounded-xl font-bold text-lg landscape:text-base hover:bg-indigo-700 active:scale-[0.98] transition-all shadow-md hover:shadow-lg">
 입장하기
 </button>
 </form>
 ) : (
 <form onSubmit={handleCenterSubmit} className="flex flex-col gap-4 landscape:gap-2">
 <div>
 <label className="block text-sm landscape:text-xs font-bold text-slate-700 mb-1 landscape:mb-0.5">센터명</label>
 <input type="text" required value={centerName} onChange={e => setCenterName(e.target.value)} placeholder="행복치매예방센터" className="w-full p-3 landscape:p-1.5 landscape:text-sm rounded-xl border border-slate-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all" />
 </div>
 {recentCenters.length > 0 && (
 <div className="landscape:mt-1">
 <label className="block text-xs font-bold text-slate-500 mb-2 landscape:mb-1">최근 로그인한 센터</label>
 <div className="flex flex-wrap gap-2">
 ...
 <button type="submit" className="mt-4 landscape:mt-1 w-full py-4 landscape:py-2 bg-slate-800 text-white rounded-xl font-bold text-lg landscape:text-base hover:bg-slate-900 active:scale-[0.98] transition-all shadow-md hover:shadow-lg">
 관리자 로그인
 </button>
 </form>
 )}
...
function RankingScreen({ scores, onRefresh }: { scores: ScoreEntry[], onRefresh?: () => void }) {
 ...
 <div className="bg-amber-400 p-6 text-center relative flex flex-col landscape:flex-row items-center justify-center gap-2">
 ...
 <button onClick={() => setTab('CENTER')} className={clsx("flex-1 py-4 text-center font-bold transition-colors text-sm sm:text-base", tab === 'CENTER' ? "bg-white text-amber-600 border-b-2 border-amber-500" : "text-slate-500 hover:bg-slate-100")}>
 센터별 랭킹
 </button>
 <button onClick={() => setTab('INDIVIDUAL')} className={clsx("flex-1 py-4 text-center font-bold transition-colors text-sm sm:text-base", tab === 'INDIVIDUAL' ? "bg-white text-amber-600 border-b-2 border-amber-500" : "text-slate-500 hover:bg-slate-100")}>
 개인전 랭킹
 </button>
 </div>

 <div className="bg-white border-b border-slate-100 px-4 py-3 overflow-x-auto whitespace-nowrap flex gap-2 shrink-0 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
 {GAME_TYPES.map(game => (
 <button
 key={game.id}
...
function LandscapeGameWrapper({ title, description, children, onBack }: { title: string, description: string, children: React.ReactNode, onBack: () => void }) {
 const [isStarted, setIsStarted] = useState(false);

 if (!isStarted) {
 return (
 <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full h-full flex flex-col items-center justify-center p-4">
 <div className="bg-white p-6 sm:p-8 landscape:p-8 rounded-3xl shadow-xl max-w-sm landscape:max-w-3xl w-full border border-slate-100 text-center landscape:text-left relative overflow-hidden flex flex-col landscape:flex-row items-center gap-2 landscape:gap-8">
 <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-indigo-500 to-purple-500 landscape:w-3 landscape:h-full" />
 
 <div className="flex flex-col items-center justify-center">
 <div className="bg-indigo-50 w-20 h-20 sm:w-24 sm:h-24 landscape:w-24 landscape:h-24 rounded-full flex items-center justify-center mb-4 landscape:mb-0 text-indigo-600 shadow-inner shrink-0">
 <Smartphone className="w-10 h-10 sm:w-12 sm:h-12 landscape:w-12 landscape:h-12 rotate-90" />
 </div>
 </div>

 <div className="flex-1 flex flex-col w-full">
 <h2 className="text-xl sm:text-2xl landscape:text-4xl font-black text-slate-800 mb-1 sm:mb-2 landscape:mb-2">{title}</h2>
 <p className="text-sm sm:text-base landscape:text-xl text-slate-500 font-medium mb-4 landscape:mb-4">두뇌 훈련 게임</p>
 
 <div className="bg-slate-50 rounded-2xl p-3 sm:p-4 landscape:p-6 mb-6 landscape:mb-6 border border-slate-100">
 <p className="text-sm sm:text-base landscape:text-2xl text-slate-700 leading-relaxed font-bold">
 {description}
 </p>
 <p className="text-indigo-600 font-black text-base sm:text-lg mt-2 bg-indigo-100/50 py-1.5 sm:py-2 px-4 rounded-xl inline-block landscape:hidden">
 스마트폰을 가로로 돌려주세요!
 </p>
 </div>

 <div className="flex gap-3 justify-center landscape:justify-start">
 <button onClick={onBack} className="flex-1 landscape:flex-none landscape:px-8 py-3 sm:py-3.5 landscape:py-4 bg-slate-100 text-slate-600 rounded-xl font-bold hover:bg-slate-200 transition-colors text-sm sm:text-base landscape:text-xl">
 돌아가기
 </button>
 <button onClick={() => setIsStarted(true)} className="flex-1 landscape:flex-none landscape:px-12 py-3 sm:py-3.5 landscape:py-4 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 transition-colors shadow-md hover:shadow-lg active:scale-95 text-sm sm:text-base landscape:text-xl">
 시작하기
 </button>
 </div>
 </div>
...
function NumberGame({ onFinish, initialLevel, onSaveLevel }: { onFinish: (score: number) => void, initialLevel: number, onSaveLevel: (level: number) => void }) {
 ...
 <div className="text-base sm:text-lg font-bold text-indigo-600">{level}단계 / 50단계</div>
 {score > 0 && (
 ...
 <h2 className="text-lg sm:text-xl font-bold mb-6 sm:mb-8 landscape:mb-2 text-slate-800 text-center landscape:hidden">1부터 {getTargetCount(level)}까지 순서대로 누르세요</h2>
 ...
 <div className="grid grid-cols-4 gap-2 sm:gap-3 w-full aspect-square landscape:max-w-[50vh] landscape:max-h-[50vh] mx-auto">
 {numbers.map((num, i) => {
...
function PictureGame({ onFinish, initialLevel, onSaveLevel }: { onFinish: (score: number) => void, initialLevel: number, onSaveLevel: (level: number) => void }) {
 ...
 <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full max-w-md flex flex-col items-center relative min-h-[400px] landscape:min-h-0 landscape:h-full landscape:justify-center">
 <div className="flex justify-between items-center w-full mb-2 sm:mb-4 px-4">
 <div className="text-base sm:text-lg font-bold text-emerald-600">{level}단계 / 50단계</div>
 {score > 0 && (
 <button onClick={() => onFinish(score)} className="px-2 py-1 sm:px-3 sm:py-1.5 bg-slate-200 text-slate-700 rounded-full text-xs sm:text-sm font-bold hover:bg-slate-300 transition-colors shadow-sm">
 저장하고 끝내기
 </button>
 ...
 <h2 className="text-lg sm:text-xl font-bold mb-6 sm:mb-8 landscape:mb-2 text-slate-800 text-center landscape:hidden">
 ...
 <div className="grid grid-cols-4 gap-2 w-full aspect-square landscape:max-w-[50vh] landscape:max-h-[50vh] mx-auto">
 {cards.map((card, i) => (
...
function PuzzleGame({ onFinish, initialLevel, onSaveLevel }: { onFinish: (score: number) => void, initialLevel: number, onSaveLevel: (level: number) => void }) {
 ...
 <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full max-w-md flex flex-col items-center relative min-h-[400px] landscape:min-h-0 landscape:h-full landscape:justify-center">
 <div className="flex justify-between items-center w-full mb-2 sm:mb-4 px-4">
 <div className="text-base sm:text-lg font-bold text-amber-600">{level}단계 / 50단계</div>
 {score > 0 && (
 <button onClick={() => onFinish(score)} className="px-2 py-1 sm:px-3 sm:py-1.5 bg-slate-200 text-slate-700 rounded-full text-xs sm:text-sm font-bold hover:bg-slate-300 transition-colors shadow-sm">
 저장하고 끝내기
 </button>
 ...
 <h2 className="text-lg sm:text-xl font-bold mb-6 sm:mb-8 landscape:mb-2 text-slate-800 text-center landscape:hidden">순서대로 맞추세요</h2>
 
 {(isLevelClear || isGameOver) && (
 <LevelComplete level={level} score={levelScore} onNext={nextLevel} isGameOver={isGameOver} isLevelClear={isLevelClear} totalScore={score} onFinish={() => onFinish(score)} />
 )}

 <div className="grid grid-cols-3 gap-1.5 sm:gap-2 w-full aspect-square landscape:max-w-[50vh] landscape:max-h-[50vh] mx-auto bg-slate-200 p-1.5 sm:p-2 rounded-2xl">
 {tiles.map((tile, i) => (
...
function ColorWordGame({ onFinish, initialLevel, onSaveLevel }: { onFinish: (score: number) => void, initialLevel: number, onSaveLevel: (level: number) => void }) {
 ...
 <div className="w-full flex flex-col landscape:flex-row items-center justify-center gap-4 flex-1">
 ...
 <p className="text-sm sm:text-base text-slate-500 mb-2 text-center landscape:hidden">5번 맞추면 다음 단계로 넘어갑니다!</p>
 <div className="flex items-center justify-center gap-4 mb-4 landscape:hidden">
 <p className="text-sm font-bold text-rose-500">현재 단계 맞춘 횟수: {correctCount} / 5</p>
 <p className="text-sm font-bold text-slate-500 bg-slate-100 px-3 py-1 rounded-full shadow-inner">진행 시간: {elapsedTime}초</p>
 </div>
 <div className="h-24 landscape:h-40 flex items-center justify-center mb-6 landscape:mb-0">
 <span style={{ color: ink.color }} className="text-6xl sm:text-7xl landscape:text-8xl font-black tracking-widest drop-shadow-sm">
 {word.name}
 </span>
 </div>
 </div>
 ...
 <div className={`grid ${gridCols} gap-2 sm:gap-3 w-full max-w-sm landscape:max-w-lg`}>
 {activeColors.map((c, i) => (
 <button key={i} onPointerDown={() => handleColorClick(c.name)} disabled={isLevelClear}
 className="py-3 landscape:py-6 rounded-xl text-lg landscape:text-3xl font-bold text-slate-700 bg-white border-2 border-slate-200 shadow-sm hover:bg-slate-50 hover:border-slate-300 active:scale-95 transition-all"
 >
 {c.name}
 </button>
 ))}
...
function PatternGame({ onFinish, initialLevel, onSaveLevel }: { onFinish: (score: number) => void, initialLevel: number, onSaveLevel: (level: number) => void }) {
 ...
 <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full max-w-md flex flex-col items-center relative min-h-[400px] landscape:min-h-0 landscape:h-full landscape:justify-center">
 <div className="flex justify-between items-center w-full mb-2 sm:mb-4 px-4">
 <div className="text-base sm:text-lg font-bold text-purple-600">{level}단계 / 50단계</div>
 {score > 0 && (
 <button onClick={() => onFinish(score)} className="px-2 py-1 sm:px-3 sm:py-1.5 bg-slate-200 text-slate-700 rounded-full text-xs sm:text-sm font-bold hover:bg-slate-300 transition-colors shadow-sm">
 저장하고 끝내기
 </button>
 ...
 <div className="text-lg sm:text-xl font-bold text-purple-600 mb-4 sm:mb-8 landscape:mb-2 h-6 sm:h-8">
 {isShowingPattern ? "순서를 기억하세요..." : (!isLevelClear && !isGameOver ? "따라 누르세요!" : "")}
 </div>
 
 <div className="grid grid-cols-2 gap-3 sm:gap-4 w-full aspect-square max-w-[300px] landscape:max-w-[45vh] landscape:max-h-[45vh] mx-auto p-3 sm:p-4 bg-slate-200 rounded-3xl">
 {colors.map((color) => (
...
function MathGame({ onFinish, initialLevel, onSaveLevel }: { onFinish: (score: number) => void, initialLevel: number, onSaveLevel: (level: number) => void }) {
 ...
 <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full max-w-md landscape:max-w-3xl flex flex-col items-center relative min-h-[400px] landscape:min-h-0 landscape:h-full landscape:justify-center">
 <div className="flex justify-between items-center w-full mb-2 sm:mb-4 px-4">
 <div className="text-base sm:text-lg font-bold text-cyan-600">{level}단계 / 50단계</div>
 {score > 0 && (
 <button onClick={() => onFinish(score)} className="px-2 py-1 sm:px-3 sm:py-1.5 bg-slate-200 text-slate-700 rounded-full text-xs sm:text-sm font-bold hover:bg-slate-300 transition-colors shadow-sm">
 저장하고 끝내기
 </button>
 ...
 <div className="w-full flex flex-col landscape:flex-row items-center justify-center gap-6 flex-1">
 <div className="flex flex-col items-center justify-center landscape:w-3/5 w-full">
 <h2 className="text-lg sm:text-xl font-bold mb-4 sm:mb-6 text-slate-800 text-center landscape:hidden">두뇌 회전 사칙연산</h2>
 <div className="py-10 sm:py-14 landscape:py-6 px-4 flex items-center justify-center mb-6 sm:mb-8 landscape:mb-0 w-full bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
 <span className="text-4xl sm:text-6xl md:text-7xl landscape:text-7xl font-black text-indigo-600 tracking-wider drop-shadow-md whitespace-nowrap">
 {equation.num1} {equation.op} {equation.num2} = ?
 </span>
 </div>
 </div>

 <div className="w-full landscape:w-2/5 flex items-center justify-center">
 <div className="grid grid-cols-2 gap-2 sm:gap-3 w-full max-w-[240px] landscape:max-w-[400px] mx-auto">
 {options.map((opt, i) => (
 <button key={i} onPointerDown={() => handleAnswer(opt)} disabled={isLevelClear}
 className="py-1.5 sm:py-2 landscape:py-6 rounded-lg text-base sm:text-lg landscape:text-4xl font-bold text-slate-700 bg-white border-2 border-slate-200 shadow-sm hover:bg-cyan-50 hover:border-cyan-300 hover:text-cyan-700 active:scale-95 transition-all"
 >
 {opt}
 </button>
 ))}
...
function OddOneOutGame({ onFinish, initialLevel, onSaveLevel }: { onFinish: (score: number) => void, initialLevel: number, onSaveLevel: (level: number) => void }) {
 ...
 const targetSize = Math.min(2 + Math.floor((lvl - 1) / 3), 6); // 2 to 6
 const targetItems = targetSize * targetSize;
 
 // 세로(row)는 최대 4칸으로 제한, 가로(col)로 늘어남
 const r = Math.min(Math.ceil(Math.sqrt(targetItems)), 4);
 const c = Math.ceil(targetItems / r);
 
 setGridRows(r);
 setGridCols(c);
 
 const totalItems = r * c;
 ...
 <h2 className="text-lg sm:text-xl font-bold mb-6 text-slate-800 text-center">다른 하나를 찾으세요!</h2>
 </div>

 <div className="w-full landscape:w-full flex items-center justify-center px-2">
 <div 
 className="grid gap-1.5 sm:gap-2 w-full mx-auto justify-center" 
 style={{ 
 gridTemplateColumns: `repeat(${gridCols}, minmax(0, 1fr))`,
 maxWidth: `min(100%, ${gridCols * 6}rem)`
 }}
 >
 {items.map(item => (
 <button
 ...
 className={clsx(
 "aspect-square bg-white shadow-sm border border-slate-200 flex items-center justify-center hover:bg-slate-50 active:scale-95 transition-all",
 gridCols > 6 ? "text-xl sm:text-2xl landscape:text-4xl rounded-lg" : "text-2xl sm:text-3xl landscape:text-5xl rounded-xl"
 )}
 >
 {item.emoji}
 </button>
...
function RPSBrainGame({ onFinish, initialLevel, onSaveLevel }: { onFinish: (score: number) => void, initialLevel: number, onSaveLevel: (level: number) => void }) {
 ...
 <div className="text-6xl landscape:text-9xl mb-4">{opponent}</div>
 <h2 className={clsx("text-2xl sm:text-3xl landscape:text-5xl font-black", 
 ...
 {choices.map(c => (
 <button key={c} onClick={() => handleChoice(c)} className="w-24 h-24 landscape:w-32 landscape:h-32 bg-white rounded-2xl shadow-md border-2 border-slate-100 text-5xl landscape:text-7xl hover:bg-slate-50 hover:scale-105 active:scale-95 transition-all">
 {c}
 </button>
 ))}
 </div>
...
function InitialQuizGame({ onFinish, initialLevel, onSaveLevel }: { onFinish: (score: number) => void, initialLevel: number, onSaveLevel: (level: number) => void }) {
 ...
 <div className="text-5xl landscape:text-8xl font-black text-slate-800 mb-2 landscape:mb-6 tracking-widest">{currentWord.initial}</div>
 <div className="text-lg landscape:text-2xl text-slate-500 font-medium bg-slate-100 py-2 landscape:py-4 px-4 landscape:px-8 rounded-full inline-block">힌트: {currentWord.hint}</div>
 <p className="text-slate-400 mt-4 font-bold landscape:hidden">{round} / 3 문제</p>
 </div>
 </div>

 <div className="w-full landscape:w-1/2 flex items-center justify-center">
 <div className={clsx("grid gap-2 w-full max-w-sm landscape:max-w-lg", options.length >= 8 ? "grid-cols-3" : "grid-cols-2")}>
 {options.map(opt => (
 <button key={opt} onClick={() => handleChoice(opt)} className={clsx("bg-white rounded-2xl shadow-sm border-2 border-slate-100 font-bold text-slate-700 hover:bg-indigo-50 hover:border-indigo-300 hover:text-indigo-700 active:scale-95 transition-all", options.length >= 8 ? "py-2 landscape:py-4 text-lg landscape:text-2xl" : "py-4 landscape:py-6 text-2xl landscape:text-4xl")}>
 {opt}
 </button>
 ))}
 </div>
...
function FlankerTaskGame({ onFinish, initialLevel, onSaveLevel }: { onFinish: (score: number) => void, initialLevel: number, onSaveLevel: (level: number) => void }) {
 ...
 <p className="text-slate-500 mb-4 font-bold text-center landscape:hidden">{round} / 5 라운드</p>

 <div className="flex justify-center items-center gap-2 mb-8 landscape:mb-0 bg-white py-6 landscape:py-10 px-8 landscape:px-12 rounded-3xl shadow-sm border border-slate-100">
 {arrows.map((arr, i) => (
 <span key={i} className={clsx("text-4xl sm:text-5xl landscape:text-7xl font-black", i === 2 ? "text-rose-500 scale-125" : "text-slate-300")}>
 {arr}
 </span>
 ))}
 </div>
 ...
 <div className="w-full landscape:w-1/2 flex items-center justify-center">
 <div className="flex gap-4 justify-center w-full max-w-sm landscape:max-w-md">
 <button onClick={() => handleChoice('LEFT')} className="flex-1 py-6 landscape:py-12 bg-white rounded-2xl shadow-md border-2 border-slate-100 text-4xl landscape:text-6xl hover:bg-slate-50 hover:border-slate-300 active:scale-95 transition-all">
 ◀
 </button>
 <button onClick={() => handleChoice('RIGHT')} className="flex-1 py-6 landscape:py-12 bg-white rounded-2xl shadow-md border-2 border-slate-100 text-4xl landscape:text-6xl hover:bg-slate-50 hover:border-slate-300 active:scale-95 transition-all">
 ▶
 </button>
 </div>
 </div>
